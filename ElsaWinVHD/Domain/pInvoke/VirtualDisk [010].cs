        ...
        public void Attach(VirtualDiskAttachOptions options) {
            ...
            switch (res)
            {
                case NativeMethods.ERROR_SUCCESS:
                    break;
                case 32:  // Диск уже подключен
                    break;
                case NativeMethods.ERROR_ACCESS_DENIED:
                    throw new IOException("Access is denied.");
                case NativeMethods.ERROR_INVALID_HANDLE:
                    throw new InvalidOperationException("The handle is invalid.");
                case NativeMethods.ERROR_BAD_COMMAND:
                    throw new NotSupportedException("Command is not supported in current state.");
